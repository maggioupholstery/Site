...
export default async function AdminQuotesPage() {
  // ✅ Pull latest quotes (no filtering)
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="mt-6 rounded-2xl border border-zinc-900 bg-black/30 p-4">
          <div className="text-sm text-zinc-300">
            Rows: <span className="font-semibold text-zinc-100">{count}</span>
          </div>
          <div className="text-xs text-zinc-500 mt-1">
            This page is forced dynamic (no caching) so new quotes should appear immediately.
          </div>
        </div>
        ...
        <div className="mt-4 overflow-hidden rounded-2xl border border-zinc-900">
          ...
          {count === 0 ? (
            ...
            <div className="divide-y divide-zinc-900">
              {rows.map((q) => (
                <div key={q.id} className="grid grid-cols-12 px-4 py-3 text-sm">
                  ...
                  <div className="col-span-2 text-zinc-100 font-semibold">
                    {fmtMoneyRange(q.estimate)}
                  </div>
                </div>
              ))}
            </div>
